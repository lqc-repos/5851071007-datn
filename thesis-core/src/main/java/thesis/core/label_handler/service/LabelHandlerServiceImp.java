...
//        CSVExporter.exportTfIdfRate(articleIdfInfos);
        divideNLPLabel(articleIdfInfos);
        return Optional.of(Boolean.TRUE);
    }

    private void divideNLPLabel(List<ArticleIdfInfo> articleIdfInfos) {
        Map<String, NLPLabel> nlpLabelMap = nlpLabelService.getMany(CommandCommonQuery.builder()
                .page(1)
                .size(Integer.MAX_VALUE)
                .build()).stream().collect(Collectors.toMap(NLPLabel::getLabel, nlpLabel -> nlpLabel));
        Map<String, PERLabel> perLabelMap = perLabelService.getMany(CommandCommonQuery.builder()
                .page(1)
                .size(Integer.MAX_VALUE)
                .build()).stream().collect(Collectors.toMap(PERLabel::getLabel, perLabel -> perLabel));
        Map<String, ORGLabel> orgLabelMap = orgLabelService.getMany(CommandCommonQuery.builder()
                .page(1)
                .size(Integer.MAX_VALUE)
                .build()).stream().collect(Collectors.toMap(ORGLabel::getLabel, orgLabel -> orgLabel));
        Map<String, LOCLabel> locLabelMap = locLabelService.getMany(CommandCommonQuery.builder()
                .page(1)
                .size(Integer.MAX_VALUE)
        ...
        log.info("=== start insert custom labels");
        nlpLabelService.addMany(nlpLabelMap.values().stream().toList());
        perLabelService.addMany(perLabelMap.values().stream().toList());
        orgLabelService.addMany(orgLabelMap.values().stream().toList());
        locLabelService.addMany(locLabelMap.values().stream().toList());
        log.info("=== end insert custom labels");
    }
}
